...
# Where the build sits. It is `/app/dist` in the image and nowhere else, so the
# variable exists for one reason: the mount-path check runs this very script
# over a checkout's `dist`, and a test that stamps the base with a copy of this
# logic proves nothing about the copy that ships.
dist="${DIST_DIR:-/app/dist}"
root="${SERVER_ROOT:-/public}"
mkdir -p "$root"
cp -R "$dist/." "$root/"

# Where this deployment answers: `/` on a host of its own, `/osiris/` as one
# tool among several on a shared one. The build carries no mount, so this is the
# only thing that differs between the two — one image, one tag, two addresses,
# no rebuild.
base_path="${BASE_PATH:-/}"
case "$base_path" in
  /*) ;;
